...
public function newsinfo($keyword,$s="",$f=""){
	...
	*/
	if($s !="")
    {
	   $sqltypeid=mysql_query("select * from `tb_accounts` where `weixinid`='$s'");
	   $typerow=mysql_fetch_array($sqltypeid);
    ...
    $keywordssql=mysql_query("select `aid`,`pipei` from `dede_addonweixin10` where `pipei` !='' and typeid='$typeid'");
	...
	if($ids)
    {
	  ...
	  $sql=mysql_query("SELECT title,weixinpic,zhaiyao,weixinurl,pipei,shuxing FROM `dede_addonweixin10` arc where arc.aid in ($idstring) and arc.typeid='$typeid' order by arc.flag desc,arc.aid desc limit 4");
		...
		while($row2 = mysql_fetch_array($sql))
		{
           if($row2['shuxing']=="文字")
           {
				    ...
				    $rows2=$stringfind;
                                    ...
                                    {
                                          $bynow=time();
                                          $insql= mysql_query("insert into `dede_addonweixin_norecord`(keyword,addtime,toname,fname,replyclass,reply) values('$keyword','$bynow','$typerow[typename]','$f','文本','1');");
                                    }
		   }
		   else
           { 
			   if($row2)
				{
				  ...
				  $rows2['leixing']='news';
                                          ...
                                          $bynow=time();
                                          $insql= mysql_query("insert into `dede_addonweixin_norecord`(keyword,addtime,toname,fname,replyclass,reply) values('$keyword','$bynow','$typerow[typename]','$f','图文','1');");
...
public function keysinfo($keyword,$s="",$f=""){
    $getword=$keyword;	
    $keyword=strtolower($keyword);//转换为小写
    $this->dbsql(); 
    ...
    $tagid=$this->GetTagid($tagkeyword);
	  ...
	  $sql5=mysql_query("select * from (select * from (SELECT aid,title,weixinpic,zhaiyao,weixinurl,pipei,shuxing,flag,tag FROM `dede_addonweixin10` arc where $addsql  and arc.shuxing like '单图文'  order by arc.flag desc,arc.aid desc) as temp1 union select * from (SELECT aid,title,weixinpic,zhaiyao,weixinurl,pipei,shuxing,flag,tag FROM `dede_addonweixin10` arc where  FIND_IN_SET('$tagid', arc.tag)>0 and arc.shuxing like '单图文'  order by arc.flag desc,arc.aid desc) as temp2) as temp3 order by flag desc,aid desc limit 8");
        ...
        if($rows3)
        {
            ...
            $rows3['type']='news';
          $bynow=time();
          $insql= mysql_query("insert into `dede_addonweixin_norecord`(keyword,addtime,toname,fname,replyclass,reply) values('$getword','$bynow','$typerow[typename]','$f','标题关键词','1');");
        }
