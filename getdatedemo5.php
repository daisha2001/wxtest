...
public function newsinfo($keyword){
    ...
    $keywordssql=mysql_query("select `aid`,`pipei` from `dede_addonweixin10` where `pipei` !=''");
	...
	if($ids)
    {
	  ...
	  $sql=mysql_query("SELECT title,weixinpic,zhaiyao,weixinurl,pipei,shuxing FROM `dede_addonweixin10` arc where arc.aid in ($idstring) order by arc.aid desc limit 4");
